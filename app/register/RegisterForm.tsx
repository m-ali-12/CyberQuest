          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="YourHackerName" required minLength={2}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPass ? 'text' : 'password'} value={form.password} onChange={e => setForm({...form, password: e.target.value})} placeholder="Min 8 characters" required minLength={8}
              ...
              {form.password && (
                <div className="mt-2">
                  <div className="flex gap-1 mb-1">
                    {[0,1,2,3].map(i => <div key={i} className={`h-1 flex-1 rounded ${i < strength ? STRENGTH_COLORS[strength-1] : 'bg-cyber-border'} transition-all`} />)}
                  </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" value={form.confirm} onChange={e => setForm({...form, confirm: e.target.value})} placeholder="Repeat password" required
                  className="w-full bg-black/30 border border-cyber-border rounded-lg pl-10 pr-10 py-3 text-white placeholder-gray-600 font-mono text-sm focus:outline-none focus:border-cyber-green/50 transition-colors" />
                {form.confirm && <CheckCircle className={`absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 ${form.password === form.confirm ? 'text-cyber-green' : 'text-cyber-red'}`} />}
              </div>
            </div>
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="text-gray-400 text-sm font-mono leading-relaxed">
                I agree to the <Link href="/terms" className="text-cyber-green hover:underline">Terms</Link> and <Link href="/privacy" className="text-cyber-green hover:underline">Privacy Policy</Link>
              </span>
            </label>
            <button type="submit" disabled={loading} className="w-full btn-cyber-solid py-3 rounded-lg font-bold font-mono text-sm">
              {loading ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" />Creating...</span> : '> Initialize Account'}
            </button>
          </form>
          <p className="text-center text-gray-500 text-sm font-mono mt-6">
            Already a member? <Link href="/login" className="text-cyber-green hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
