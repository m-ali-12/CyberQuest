...
import { Shield, BookOpen, ChevronRight, Lock, Play } from 'lucide-react';
    ...
    <div className="min-h-screen bg-cyber-dark cyber-grid">
      <nav className="border-b border-cyber-border bg-cyber-dark/90 backdrop-blur-xl px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex gap-3">
          <Link href="/login" className="text-gray-400 hover:text-white font-mono text-sm px-4 py-2">Login</Link>
          ...
          {COURSES.map((c) => (
            <div key={c.title} className="cyber-card rounded-xl border border-cyber-border hover:border-cyber-green/40 transition-all overflow-hidden group">
              <div className="p-5">
                ...
                <Link href={c.free ? '/register' : '/register'} className={`w-full text-center text-xs font-mono py-2 rounded-lg flex items-center justify-center gap-1.5 transition-all ${c.free ? 'btn-cyber-solid' : 'btn-cyber'}`}>
