    ...
    try {
      ...
      if (res?.error) {
        if (res.error.startsWith('BANNED:')) {
          ...
          toast.error(`⛔ Account temporarily locked for ${minutes} minutes`);
        } else {
          toast.error('Invalid email or password');
        }
      } else {
        toast.success('Welcome back, Hacker! 🛡️');
          ...
          <div className="mb-6 p-4 rounded-xl border border-cyber-red/50 bg-cyber-red/10 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-xs font-mono">
                Too many active sessions detected. Locked for:{' '}
                <span className="text-cyber-red font-bold">{formatCountdown(countdown)}</span>
              </p>
              <p className="text-gray-500 text-xs mt-1">Each violation increases lock duration automatically.</p>
            </div>
          </div>
        )}
        ...
        <div className="cyber-card rounded-2xl p-8 border border-cyber-border">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <Link href="/forgot-password" className="text-cyber-green text-sm font-mono hover:underline">Forgot password?</Link>
            </div>

            <button
              type="submit"
              disabled={loading || countdown > 0}
              className={`w-full py-3 rounded-lg font-bold font-mono text-sm transition-all ${
                countdown > 0
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center">
              <span className="bg-cyber-card px-3 text-gray-600 text-xs font-mono">OR CONTINUE WITH</span>
            </div>
          </div>
          ...
          <p className="text-center text-gray-500 text-sm font-mono mt-6">
            New hacker?{' '}
            <Link href="/register" className="text-cyber-green hover:underline">Create account</Link>
          </p>
        </div>
